(function(){
  'use strict';
  if(window.__keibaRaceMetaGuardV208)return;
  window.__keibaRaceMetaGuardV208=true;
  ...
  function blockResults(blocked){
    resultIds.forEach(id=>{
      ...
      if(blocked){
        if(el.dataset.v208Blocked!=='1')el.dataset.v208Display=el.style.display||'';
        el.dataset.v208Blocked='1';
        el.style.display='none';
      }else if(el.dataset.v208Blocked==='1'){
        el.style.display=el.dataset.v208Display||'';
        delete el.dataset.v208Display;
        delete el.dataset.v208Blocked;
      }
    });
  }
  function banner(text,state){
    let el=document.getElementById('raceMetaGuardV208');
    if(!el){
      el=document.createElement('div');el.id='raceMetaGuardV208';
  ...
  function cacheKey(rid){return 'keiba_race_meta_v208_'+rid;}
  function loadMetaCache(rid){
    for(const key of [cacheKey(rid),'keiba_race_meta_v206_'+rid]){
  ...
  async function resolveMeta(url){
    ...
    const jobs=fallback.map(async u=>{
      try{return parseMeta(await fetchText(u,3200),course);}catch(e){return null;}
    });
  ...
  async function waitImportDone(token,beforeBulk,beforeImport,beforeDiag){
    const started=Date.now();let lastBulk='',stableSince=0;
    for(let i=0;i<150;i++){
      ...
      if(ready&&((success&&changed)||(changed&&stable)||(success&&elapsed>1800)||(!btn?.disabled&&stable&&elapsed>2200))){await sleep(120);return true;}
      await sleep(180);
    }
    return false;
  }
  ...
  function mount(){
    ...
    if(btn.dataset.raceMetaGuardV208)return;btn.dataset.raceMetaGuardV208='1';
