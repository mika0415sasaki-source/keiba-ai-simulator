(function(){
  'use strict';
  if(window.__keibaRuntimeV223)return;
  window.__keibaRuntimeV223=true;

  const SUPABASE_URL='https://qhzccahbevnqaoxdfnbx.supabase.co';
  const ANON_JWT='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInJlZiI6InFoemNjYWhiZXZucWFveGRmbmJ4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODc2MTExNjEsImV4cCI6MjEwMzE4NzE2MX0.7RnMtCdjJTJEdbeJHMz2lQ9QIge7UvOpzLVJtdNJnFY';
  let seq=0;
  const cache=new Map();
  ...
  function showStatus(text,state){
    let e=document.getElementById('raceMetaV223Status');
    if(!e){
      e=document.createElement('div');
      e.id='raceMetaV223Status';
  ...
  async function getMeta(raceId){
    ...
    for(let i=0;i<3;i++){
      ...
      catch(e){last=e;if(i<2)await new Promise(function(r){setTimeout(r,600);});}
    }
  ...
  function emitEntryImported(x,raceId){
    try{
      const detail={
        raceId:String(raceId||''),
        raceNo:Number(String(raceId||'').slice(-2))||0,
        course:String(x.course||''),surface:String(x.surface||''),variant:String(x.variant||''),
        distance:Number(x.distance||0),going:String(x.going||''),importedAt:new Date().toISOString()
      };
      window.dispatchEvent(new CustomEvent('keiba:entry-imported',{detail:detail}));
    }catch(e){}
  }
  ...
  function mount(){
    ...
    if(b.dataset.runtimeV223)return;
    b.dataset.runtimeV223='1';
