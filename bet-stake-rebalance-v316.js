(()=>{
  if(window.__betStakeRebalanceV316)return;
  window.__betStakeRebalanceV316=true;
  ...
  function rebalance(){
    ...
    // まず、残予算で「当たっても総予算割れ」の既存買い目を黒字化できるなら優先して補強。
    ...
    // 残りは本線へ。点数は増やさず、中心買い目の厚みだけ増やす。
    const maxStake=Math.max(300,Math.min(600,Math.ceil(cap*.20/100)*100));
    while(total+100<=cap){
      const candidates=order.filter(o=>o.p>=6500&&(+picks[o.i].stake||0)<maxStake);
  ...
  function install(){
    ...
    if(typeof g!=='function'||g.__betStakeRebalanceV316||!g.__betStrategyV315)return false;
    ...
    wrapped.__betStakeRebalanceV316=true;
