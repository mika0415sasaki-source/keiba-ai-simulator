(function(){
  'use strict';
  if(window.__keibaRaceMetaGuardV200)return;
  window.__keibaRaceMetaGuardV200=true;
  ...
  function banner(text,state){
    let el=document.getElementById('raceMetaGuardV200');
    if(!el){
      el=document.createElement('div');el.id='raceMetaGuardV200';
  ...
  async function fetchText(url,ms){
    const ctrl=new AbortController();const timer=setTimeout(function(){ctrl.abort();},ms||7000);
  ...
  function parseMeta(text,forcedCourse){
    ...
    const pats=[
      ...
      /([1-4]\d{3})\s*メートル\s*[（(]\s*(芝|ダート|ダ)([^）)]{0,24})[）)]/i
    ];
  ...
  async function resolveMeta(url){
    ...
    for(const p of pages){
      for(const u of ['https://r.jina.ai/'+p,p]){
        try{const meta=parseMeta(await fetchText(u,7000),course);if(meta)return meta;}catch(e){}
      }
    }
  ...
  async function setSelect(el,value){
    ...
    el.value=String(value);el.dispatchEvent(new Event('change',{bubbles:true}));await sleep(40);return true;
  }
  ...
  async function verify(mySeq,url){
    try{
      ...
      if(mySeq!==seq)return;setButtons(true);
  ...
  function mount(){
    ...
    if(btn.dataset.raceMetaGuardV200)return;
    btn.dataset.raceMetaGuardV200='1';
    btn.addEventListener('click',function(){
      seq++;const mySeq=seq,url=inputUrl();setButtons(true);
