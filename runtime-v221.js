(function(){
  'use strict';
  if(window.__keibaRuntimeV221)return;
  window.__keibaRuntimeV221=true;
  ...
  function showStatus(text,state){
    let e=document.getElementById('raceMetaV221Status');
    if(!e){
      e=document.createElement('div');
      e.id='raceMetaV221Status';
  ...
  function setAnalysisEnabled(enabled){
    ['navAnalyze','navPlay','navBatch','play','batch'].forEach(function(id){
      ...
      if(e){
        e.disabled=!enabled;
        e.style.opacity=enabled?'':'0.45';
      }
    });
  }

  function isDone(t){
    return /\d+頭を?取り込みました|\d+頭を反映しました|直接反映しました/.test(String(t||''));
  }

  function isLoading(t){
    return /取得中|読込中|読み込み中|解析中|反映中|①\s*netkeiba競馬新聞を取得中/.test(String(t||''));
  }
  ...
  async function waitForFreshImport(my,before){
    ...
    while(Date.now()-started<180000){
      ...
      const fresh=(sawChange||sawLoading);
      if(fresh&&isDone(now)){
  ...
  function selectValue(el,value){
    ...
    const o=Array.from(el.options||[]).find(function(x){
      return String(x.value)===s||String(x.textContent).replace(/m$/i,'')===s;
    });
    if(!o)return false;
    el.value=o.value;
    return true;
  ...
  async function handleImport(raceId,before){
    patchIntro();
    if(!raceId){
      showStatus('⚠ race_idを取得できません','warn');
      setAnalysisEnabled(false);
      return;
    }
    const my=++seq;
    setAnalysisEnabled(false);
    showStatus('出馬表の取得完了を待っています…','pending');

    const done=await waitForFreshImport(my,before);
    if(my!==seq)return;
    if(!done){
      showStatus('⚠ 出馬表取得の完了を確認できません','warn');
      return;
    }
    ...
    let x=null;
    let err='';
    try{x=await getMeta(raceId);}catch(e){err=String(e&&e.message||e);}
    if(my!==seq)return;

    if(x&&applyMeta(x,raceId))return;
    setAnalysisEnabled(false);
    showStatus('⚠ レース条件取得失敗'+(err?' ['+err+']':'')+'。AI計算を停止しました','warn');
  }
  ...
  function mount(){
    ...
    if(b.dataset.runtimeV221)return;
    b.dataset.runtimeV221='1';

    // captureで「今回のクリック前」の状態を保存。古い12頭完了表示を今回の完了と誤認しない。
