(()=>{
  if(window.__betStrategyV322)return;
  window.__betStrategyV322=true;
  ...
  function wideCandidateCount(cap,poolLen){
    if(cap<=500)return Math.min(5,poolLen);
    if(cap<900)return Math.min(poolLen,Math.max(5,Math.floor(cap/100)));
    return poolLen;
  }
  ...
  function trioPools(a,mode,cap){
    ...
    const stdKeys=standardFormationKeys(a);
    const standard=full.filter(x=>stdKeys.has(x.key));
    const expanded=full.filter(x=>!stdKeys.has(x.key));
    const baseOrder=stableOrder(standard,a,mode);
    const extraOrder=stableOrder(expanded,a,mode);
  ...
  function recommendedCount(cap,mode,candidateLen){
    if(mode==='wide')return Math.min(candidateLen,Math.max(1,Math.floor(cap/100)));
    if(mode==='axis'){
      ...
      return Math.min(candidateLen,10);
    }
    if(cap<=900)return Math.min(candidateLen,Math.min(9,Math.floor(cap/100)));
    if(cap<=1200)return Math.min(candidateLen,10);
    if(cap<=1500)return Math.min(candidateLen,12);
    if(cap<=2000)return Math.min(candidateLen,13);
    return Math.min(candidateLen,15);
  }
  ...
  function allocate(items,cap,mode){
    const st=new Array(items.length).fill(100);let total=items.length*100;if(total>=cap)return {st,total};
    const order=items.map((x,i)=>({i,p:priority(x,mode)})).sort((a,b)=>b.p-a.p),maxStake=mode==='wide'?300:300;
    let guard=0;
    while(total+100<=cap&&guard++<500){
      const candidates=order.filter(o=>st[o.i]<maxStake&&(mode==='wide'?o.p>=7000:o.p>=6500));
      if(!candidates.length)break;
      candidates.sort((x,y)=>st[x.i]-st[y.i]||y.p-x.p);
      st[candidates[0].i]+=100;total+=100;
    }
    return {st,total};
  }
  ...
  function generate(){
    ...
    if(d.mode==='wide'){
      const full=widePool(a),n=wideCandidateCount(d.cap,full.length);candidates=full.slice(0,n);ordered=candidates;
    }else{
  ...
  function install(){
    try{
      ...
      const wrapped=function(){try{return generate()}catch(e){console.warn('bet strategy v322',e);return []}};
      wrapped.__betStrategyV315=true;wrapped.__betStrategyV322=true;
