(function(){
  'use strict';
  if(window.__keibaRaceMetaGuardV204)return;
  window.__keibaRaceMetaGuardV204=true;
  ...
  function blockResults(blocked){
    resultIds.forEach(id=>{
      ...
      if(blocked){
        if(el.dataset.v204Blocked!=='1')el.dataset.v204Display=el.style.display||'';
        el.dataset.v204Blocked='1';
        el.style.display='none';
      }else if(el.dataset.v204Blocked==='1'){
        el.style.display=el.dataset.v204Display||'';
        delete el.dataset.v204Display;
        delete el.dataset.v204Blocked;
      }
    });
  }
  function banner(text,state){
    let el=document.getElementById('raceMetaGuardV204');
    if(!el){
      el=document.createElement('div');el.id='raceMetaGuardV204';
  ...
  function mount(){
    ...
    if(btn.dataset.raceMetaGuardV204)return;btn.dataset.raceMetaGuardV204='1';
