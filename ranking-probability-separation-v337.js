(()=>{
  ...
  function marketAdjustment(h){
    ...
    const marketProb=100/o;
    return clamp((marketProb-10)*.06,-1.2,1.8);
  }

  // v55本体ではAI指数に単勝オッズ由来の微補正が入る。
  // v337ではそれを外し、AI順位を能力・適性だけの順位へ戻す。
  function separateAiScore(){
    ...
    rows.forEach(h=>{
      ...
      const before=Number(h?.score);
      const adj=marketAdjustment(h);
  ...
  function aiShares(rows){
    ...
    const sum=raw.reduce((a,b)=>a+b,0)||1;return raw.map(v=>v/sum);
  }
  ...
  function top3Probabilities(w){
    ...
    for(let i=0;i<n;i++){
      ...
      for(let j=0;j<n;j++){
        if(j===i)continue;const d2=W-w[j];if(d2<=0)continue;const pj=w[j]/W;p+=pj*wi/d2;
        for(let k=0;k<n;k++){
          if(k===i||k===j)continue;const d3=d2-w[k];if(d3<=0)continue;p+=pj*(w[k]/d2)*(wi/d3);
        }
      }
  ...
  function annotateUi(){
    ...
    if(ranking){
      ...
      if(panel){
        ...
        if(!note){note=document.createElement('div');note.id='rankingModelNoteV337';note.className='small';note.style.cssText='margin:-2px 0 10px;line-height:1.55;color:#9fb0cf';ranking.parentNode.insertBefore(note,ranking)}
      ...
      cards.forEach((card,i)=>{
        ...
        if(rank)rank.innerHTML=rank.innerHTML.replace(/^\s*\d+位\s*[　 ]*/,'').replace(/^/,'AI '+h.aiRankV337+'位　');
