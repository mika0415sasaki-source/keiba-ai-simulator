(()=>{
  ...
  function patchCards(){const hs=horseList(),root=el('horses');if(!root)return;const byName=new Map(hs.map(h=>[norm(h.name),h]));for(const card of root.querySelectorAll(':scope > .card')){const title=card.querySelector('.rank'),name=String(title?.textContent||'').replace(/^\s*\d+\s*/,'').trim(),h=byName.get(norm(name));if(!h)continue;const smalls=[...card.querySelectorAll('.small')];if(smalls[0])smalls[0].textContent=[h.sex_age||'',bodyText(h),cleanJockey(h.jockey||h.rider||''),Number.isFinite(+h.carried_weight)?`斤量 ${(+h.carried_weight).toFixed(1)}kg`:'' ].filter(Boolean).join('　');const styleLine=smalls.find(x=>/^(脚質推定：|脚質：)/.test(String(x.textContent||'')));const s=styleFor(h);if(styleLine)styleLine.textContent=`脚質：${s||'—'}${s?'（netkeiba）':'（netkeiba未取得）'}　父：${h.sire||'—'}　母：${h.dam||'—'}　母父：${h.damsire||'—'}`}}
  function installFunctionGuards(){installQualityGuard();try{window.inferStyleFromJra=h=>styleFor(h);inferStyleFromJra=h=>styleFor(h)}catch(_){}try{window.renderPaceReason=renderPace;renderPaceReason=renderPace}catch(_){} }
