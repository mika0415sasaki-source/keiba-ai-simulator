(()=>{
  ...
  function applyCurrentData(){
    const hs=horseList();
    const orderedRows=hs.map(h=>currentRowFor(h));
    const jockeyShiftMatches=orderedRows.reduce((n,x,i)=>{
      if(i<1||!x)return n;
      const prev=cleanJockey(hs[i-1]?.jockey||hs[i-1]?.rider||'');
      const cur=cleanJockey(x?.jockey);
      return n+(prev&&cur&&prev===cur?1:0);
    },0);
    const jockeyOffsetDetected=jockeyShiftMatches>=2;
    for(let hi=0;hi<hs.length;hi++){
      ...
      if(x){
        ...
        if(Number.isFinite(+x.body_weight)&&+x.body_weight>=300&&+x.body_weight<=700){h.body_weight=+x.body_weight;h.weight=+x.body_weight;h.__currentBodyWeightV370=+x.body_weight}
